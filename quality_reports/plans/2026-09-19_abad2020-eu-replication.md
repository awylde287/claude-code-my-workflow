...
| D5 | 4 traded prices between events | Quoted bid/ask/mid, no trades | **Define an *informative day* and apply Abad's filter to it unchanged.** A quoted row is informative when **the mid price changed AND the Z-spread changed**. The conjunction is what makes it right: a carried-forward price leaves the mid unchanged while the curve still moves its Z-spread, and a matrix-priced row moves the mid off the curve while its Z-spread stays flat. Only a row where both move can carry bond-specific news, which is what a trade carries in Abad. Filter: **≥ 4 informative days between consecutive rating events on the same bond**, verbatim from §2 of the paper. Secondary flags: both bid and ask unchanged; `Δmid ≈ −ModDuration × Δ(curve yield)` within tolerance (purely curve-driven). Use relative `Ask − Bid` as a liquidity control in Panel B and report it as an addition to the paper. |
| D6 | t1 = last trade before, t2 = first trade after | Daily quotes, so a literal reading of the rule gives a near-constant 1–2 day window | **Keep Abad's expandable window, but expand it over *informative* days (D5), not over quoted rows.** Abad's window is variable because TRACE bonds do not trade every day, not because a variable window is desirable; §3 says as much, citing Ederington et al. (2015) on sample size and power. Reading "a price" as "a row exists" removes the variation and leaves `W` unidentified. Reading it as "an informative day" restores Abad's intent exactly: liquid bonds collapse to (−1, +1), stale bonds expand precisely because they are stale, and `W` varies again and stays estimable. **Cap the expansion** (e.g. 20 trading days) and drop events that never reach an informative day. Robustness: fixed (−1, +1) and (−1, +5); bid instead of mid. Confirm with LSEG whether quotes are end-of-day; an announcement after the close belongs to day +1. |
